...
7.1. Autenticação
O usuário acessa public/login.php.
Insere username e senha, que são validados pela classe Auth contra a tabela usuarios.
Se válido, salva os dados na sessão ($_SESSION['auth']) e redireciona para index.php.
7.2. Navegação e Ações
Em index.php, o sistema verifica as permissões do usuário para cada ação solicitada.
As ações são processadas apenas se o usuário tiver a permissão correspondente:
...
A interface exibe apenas os elementos que o usuário tem permissão para usar.
Mensagens de feedback específicas são exibidas quando o acesso é negado.
...
Integridade de dados: Constraints como UNIQUE na placa previnem duplicidade.
Consistência: Transações garantem operações atômicas e consistentes.
Desempenho: Índices otimizam a busca e recuperação de dados.
Concorrência: Suporte a múltiplos acessos simultâneos com controle de bloqueio.
Segurança: Autenticação e autorização no nível do banco de dados.
11.2. Benefícios do Sistema de Permissões
Escalabilidade: Facilita a adição de novos perfis com conjuntos específicos de permissões.
Manutenibilidade: Permissões definidas em um único lugar, facilitando alterações.
Clareza: Código mais legível e autodocumentado, com verificações explícitas.
Segurança: Cada ação protegida individualmente, reduzindo riscos de acesso não autorizado.
Feedback ao usuário: Mensagens específicas quando uma permissão é negada.
11.3. Melhorias na Organização
Arquivo index.php na raiz: Facilita o acesso direto à aplicação.
Interface Locavel incorporada: Simplifica a estrutura de arquivos mantendo o padrão OO.
Dicas de login: Melhora a experiência de primeiro acesso com informações claras sobre credenciais.
Inicialização automática do banco: Sistema "plug and play" que cria sua própria estrutura.
